 ...
 * and WebGL methods can be called like so:
 * 
 *     context.gl.clear(GL_COLOR_BIT | GL_DEPTH_BUFFER_BIT);
 *
 * ### Error Reporting
 *
...
Jax.Context = (function() {
  function setupContext(self, options) {
    ...
    options = Jax.Util.normalizeOptions(options, WEBGL_CONTEXT_OPTIONS);
    try { self.gl = self.canvas.getContext(WEBGL_CONTEXT_NAME, options); } catch(e) { }
    if (!self.gl) throw new Error("WebGL could not be initialized!");
  }
  
  function updateFramerate(self) {
    var current_render_start = self.uptime;
    if (!self.last_render_start) self.last_render_start = self.uptime;
    ...
    self.framerate = self.frames_per_second = 1000 / self.time_to_render;
    self.last_render_start = current_render_start;
  }
  
  function updateUpdateRate(self) {
    var current_update_start = self.uptime;
    ...
    if (self.calculateUpdateRate) {
      ...
      self.updates_per_second = 1000 / self.time_to_update;
    }
  ...
  function requestFrame(func, self) {
    if (Jax.useRequestAnimFrame && self.useRequestAnimFrame)
      return requestAnimationFrame(func, self);
    else {
      var currTime = new Date().getTime();
      var timeToCall = Math.max(0, 16 - (currTime - (self._requestFrameLastTime || 0)));
      var id = setTimeout(function() { func(currTime + timeToCall); }, timeToCall);
      self._requestFrameLastTime = currTime + timeToCall;
      return id;
    }
  }
    ...
    function render(currentTime) {
      ...
      try {
        ...
        if (self.current_view) {
          self.render();
          if (self.isRendering()) {
            self.render_interval = requestFrame(render, self);
  ...
  function reloadMatrices(self) {
    ...
    self.matrix_stack.loadViewMatrix(self.player.camera.getInverseTransformationMatrix());
  ...
  var klass = Jax.Class.create({
    ...
    initialize: function(canvas, options) {
      try {
         ...
         **/
        options = options || {};
        options.alertErrors = options.alertErrors === undefined ? true : options.alertErrors;
        this.uptime = 0;
        ...
        this.player.camera.perspective({width:canvas.clientWidth || canvas.width, height:canvas.clientHeight || canvas.height});
    ...
    render: function() {
      ...
      if (this.current_view)
        ...
        this.gl.clear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
        this.world.render();
      }
    },
     ...
     *       context.matrix_stack.loadModelMatrix(mat4.IDENTITY);
     *       context.matrix_stack.multViewMatrix(this.camera.getTransformationMatrix());
     *       // do some rendering
     *     });
