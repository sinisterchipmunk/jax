...
Jax.Mesh.prototype._makeTangentBuffer = function() {
  ...
  var normals = self.data.normalBuffer;
  var vertices = self.data.vertexBuffer;
  var texcoords = self.data.textureCoordsBuffer;
  var indices = self.getIndexBuffer();
  if (!normals || !vertices || !texcoords) return null;

  var tangentBuffer = self.tangent_buffer;
  if (indices && indices.length > 0 && indices.js) indices = indices.js;
  else indices = null;
  
  var numTangents = vertices.length / 3 * 4;
  // TODO tangent buffer allocation should be handled by the mesh data
  var tangents = tangentBuffer ? tangentBuffer.js : new Float32Array(numTangents);
  if (tangents.length != numTangents) {
    tangentBuffer = null;
    tangents = new Float32Array(numTangents);
  }
  var tan1 = [], tan2 = [], a;
  ...
  if (tangentBuffer)
    ...
    self.tangent_buffer = new Jax.Buffer(GL_ARRAY_BUFFER, Float32Array, GL_STATIC_DRAW, tangents, 4);
