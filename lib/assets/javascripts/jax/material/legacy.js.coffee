class Jax.Material.Legacy extends Jax.Material.Custom
  ...
  @define 'shininess',
    get: -> @_shininess
    set: (s) ->
      @findLayer(Jax.Material.PhongSpecular)?.shininess = s
      @_shininess = s

  @define 'pcf',
    get: -> @_pcf
    set: (s) ->
      @findLayer(Jax.Material.ShadowMap)?.pcf = s
      @_pcf = s
  ...
  constructor: (options, name) ->
    ...
    Object.defineProperty @_intensity, 'ambient',
      get:     -> @_ambient
      set: (i) ->
        mat.findLayer(Jax.Material.WorldAmbient)?.intensity = i
        mat.findLayer(Jax.Material.LightAmbient)?.intensity = i
        @_ambient = i
    Object.defineProperty @_intensity, 'diffuse',
      get:     -> @_diffuse
      set: (i) ->
        mat.findLayer(Jax.Material.LambertDiffuse)?.intensity = i
        @_diffuse = i
    Object.defineProperty @_intensity, 'specular',
      get:     -> @_specular
      set: (i) ->
        mat.findLayer(Jax.Material.PhongSpecular)?.intensity = i
        @_specular = i
    ...
    Object.defineProperty @_color, 'ambient',
      get:     -> @_ambient
      set: (c) ->
        mat.findLayer(Jax.Material.WorldAmbient)?.color = Jax.Color.parse c
        mat.findLayer(Jax.Material.LightAmbient)?.color = Jax.Color.parse c
        @_ambient = c
    Object.defineProperty @_color, 'diffuse',
      get:     -> @_diffuse
      set: (c) ->
        mat.findLayer(Jax.Material.LambertDiffuse)?.color = Jax.Color.parse c
        @_diffuse = c
    Object.defineProperty @_color, 'specular',
      get:     -> @_specular
      set: (c) ->
        mat.findLayer(Jax.Material.PhongSpecular)?.color = Jax.Color.parse c
        @_specular = c
