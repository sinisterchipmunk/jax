...
Jax.Geometry.Triangle = (function() {
  var bufs = {
    cwV1: GLMatrix.vec3.create(),
    cwV2: GLMatrix.vec3.create(),
    cwV3: GLMatrix.vec3.create()
  };
  ...
  var Triangle = Jax.Class.create({
     ...
     **/
    set: function(a, b, c) {
      return this.assign(GLMatrix.vec3.clone(a), GLMatrix.vec3.clone(b), GLMatrix.vec3.clone(c));
    },
    ...
    isClockwise: function(xform) {
      ...
      var a, b, c;
      if (xform) {
        GLMatrix.vec3.transformMat4(v1, this.a, xform);
        GLMatrix.vec3.transformMat4(v2, this.b, xform);
        GLMatrix.vec3.transformMat4(v3, this.c, xform);
        a = v1;
        b = v2;
        c = v3;
      } else {
      ...
      GLMatrix.vec3.subtract(v1, b, a);
      GLMatrix.vec3.subtract(v2, c, a);
    ...
    recalculateCenter: function() {
      // (a+b+c) / 3
      GLMatrix.vec3.add(this.center, this.a, this.b);
      GLMatrix.vec3.add(this.center, this.c, this.center);
      GLMatrix.vec3.scale(this.center, this.center, 1/3);
      return this.center;
    },
    
    recalculateNormal: function() {
      var tmp = bufs.tmp = bufs.tmp || GLMatrix.vec3.create();
      GLMatrix.vec3.subtract(this.normal, this.b, this.a);
      GLMatrix.vec3.subtract(tmp, this.c, this.a);
      GLMatrix.vec3.cross(this.normal, this.normal, tmp);
    ...
    toString: function() {
      return "Triangle: "+GLMatrix.vec3.str(this.a)+"; "+GLMatrix.vec3.str(this.b)+"; "+GLMatrix.vec3.str(this.c);
    },
    ...
    intersectRay: function(O, D, cp, segmax) {
      ...
      var denom = GLMatrix.vec3.dot(p.normal, D);
      if (Math.abs(denom) < Math.EPSILON) return false;
      var t = -(p.d + GLMatrix.vec3.dot(p.normal, O)) / denom;
      if (t <= 0) return false;
      if (segmax != undefined && t > segmax) return false;

      // cp = O + t*D
      GLMatrix.vec3.copy(cp, D);
      GLMatrix.vec3.scale(cp, cp, t);
      GLMatrix.vec3.add(cp, O, cp);
    ...
    intersectSphere: function(O, radius, cp) {
      ...
      var point = this._point = this._point || GLMatrix.vec3.create();
      GLMatrix.vec3.scale(point, p.normal, dist);
      GLMatrix.vec3.subtract(point, O, point);
      if (this.pointInTri(point)) {
        if (cp) GLMatrix.vec3.copy(cp, point);
        return true;
      }

      // edge intersection detection
      var v = bufs.v = bufs.v || [];
      var u = bufs.u = bufs.u || GLMatrix.vec3.create();
      var pa = bufs.pa = bufs.pa || GLMatrix.vec3.create();
      var tmp = bufs.tmp = bufs.tmp || GLMatrix.vec3.create();
      var radsquared = radius*radius;

      v[0] = this.a; v[1] = this.b; v[2] = this.c; v[3] = this.a;
      for (var i = 0; i < 3; i++) {
        GLMatrix.vec3.subtract(u, v[i+1], v[i]);
        GLMatrix.vec3.subtract(pa, O, v[i]);
        var s = GLMatrix.vec3.dot(u, pa) / GLMatrix.vec3.dot(u, u);

        if (s < 0) GLMatrix.vec3.copy(tmp, v[i]);
        else if (s > 1) GLMatrix.vec3.copy(tmp, v[i+1]);
        else {
          GLMatrix.vec3.scale(tmp, u, s);
          GLMatrix.vec3.add(tmp, v[i], tmp);
        }
        if (cp) GLMatrix.vec3.copy(cp, tmp);

        GLMatrix.vec3.subtract(tmp, O, tmp);
        var sq_dist = GLMatrix.vec3.dot(tmp, tmp);
    ...
    pointInTri: function(P) {
      var u = this._u = this._u || GLMatrix.vec3.create();
      var v = this._v = this._v || GLMatrix.vec3.create();
