...
Jax.Geometry.Line = (function() {
  ...
  var Line = Jax.Class.create({
    ...
    contains: function(point) {
      ...
      var ba = GLMatrix.vec3.subtract(bufs.ba || (bufs.ba = GLMatrix.vec3.create()), this.b, this.a);
      var pa = GLMatrix.vec3.subtract(bufs.pa || (bufs.pa = GLMatrix.vec3.create()), point,  this.a);
      var ab = GLMatrix.vec3.subtract(bufs.ab || (bufs.ab = GLMatrix.vec3.create()), this.a, this.b);
      var pb = GLMatrix.vec3.subtract(bufs.pb || (bufs.pb = GLMatrix.vec3.create()), point,  this.b);
      
      GLMatrix.vec3.normalize(ba, ba);
      GLMatrix.vec3.normalize(pa, pa);
      GLMatrix.vec3.normalize(ab, ab);
      GLMatrix.vec3.normalize(pb, pb);
    ...
    intersectLineSegment: function(line, dest) {
      var u = GLMatrix.vec3.subtract(GLMatrix.vec3.create(), this.b, this.a);
      var v = GLMatrix.vec3.subtract(GLMatrix.vec3.create(), line.b, line.a);
      var w = GLMatrix.vec3.subtract(GLMatrix.vec3.create(), this.a, line.a);
      ...
      if (Math.abs(D) < Math.EPSILON) { // S1 and S2 are parallel
        ...
        var du = GLMatrix.vec3.dot(u, u);
        var dv = GLMatrix.vec3.dot(v, v);
        if (du == 0 && dv == 0) { // both segments are points
          if (!Math.equalish(this.a, line.a)) // they are distinct points
            return Jax.Geometry.DISJOINT;
          // they are the same point
          if (dest)
            if (isVec3) GLMatrix.vec3.copy(dest, line.a);
            else GLMatrix.vec3.copy(dest.a, line.a);
        ...
        if (du == 0) { // +this+ is a single point
          if (!line.contains(this.a)) // but is not in S2
            return Jax.Geometry.DISJOINT;
          if (dest)
            if (isVec3) GLMatrix.vec3.copy(dest, this.a);
            else GLMatrix.vec3.copy(dest.a, this.a);
        ...
        if (dv == 0) { // +line+ is a single point
          if (!this.contains(line.a)) // but is not in this line
            return Jax.Geometry.DISJOINT;
          if (dest)
            if (isVec3) GLMatrix.vec3.set(dest, line.a);
            else GLMatrix.vec3.set(dest.a, line.a);
          // vec3.set(line.b, dest.b);
          return Jax.Geometry.INTERSECT;
        }
        // they are colinear segments - get overlap (or not)
        var t0, t1; // endpoints of +this+ in eqn for +line+
        var w2 = GLMatrix.vec3.subtract(GLMatrix.vec3.create(), this.a, line.a);
        if (v[0] != 0) {
          t0 = w[0] / v[0];
          t1 = w2[0] / v[0];
        } else {
        ...
        if (t0 == t1) {
          ...
          if (line) {
            if (dest) {
              ...
              GLMatrix.vec3.add(dest_a, line.a, GLMatrix.vec3.scale(dest_a, v, t0));
            }
            return Jax.Geometry.INTERSECT;
          }
        }
        ...
        if (dest) {
          if (isVec3) {
            ...
            GLMatrix.vec3.add(dest, line.a, GLMatrix.vec3.scale(dest, v, t0));
            GLMatrix.vec3.add(tmp,  line.b, GLMatrix.vec3.scale(tmp,  v, t1));
            GLMatrix.vec3.scale(dest, dest, 0.5);
          } else {
            GLMatrix.vec3.add(dest.a, line.a, GLMatrix.vec3.scale(dest.a, v, t0));
            GLMatrix.vec3.add(dest.b, line.b, GLMatrix.vec3.scale(dest.b, v, t1));
          }
        }
      ...
      if (dest) GLMatrix.vec3.add(dest, this.a, GLMatrix.vec3.scale(dest, u, sI));
      return Jax.Geometry.INTERSECT;
    },
