...
Jax.Geometry.Plane = (function() {
  ...
  var Plane = Jax.Class.create({
    toString: function() {
      return "[Plane normal:"+GLMatrix.vec3.str(this.normal)+"; D:"+this.d+"]";
    },
    ...
    intersectTriangle: function(t, line) {
      ...
      var p1 = bufs.tri_p1 = bufs.tri_p1 || GLMatrix.vec3.create(),
          p2 = bufs.tri_p2 = bufs.tri_p2 || GLMatrix.vec3.create();
    ...
    intersectLineSegment: function(line, point) {
      var u = GLMatrix.vec3.subtract((bufs.lineseg_u = bufs.lineseg_u || GLMatrix.vec3.create()), line.b, line.a);
      var w = GLMatrix.vec3.subtract((bufs.lineseg_w = bufs.lineseg_w || GLMatrix.vec3.create()), line.a, this.point);
      var D =  GLMatrix.vec3.dot(this.normal, u);
      var N = -GLMatrix.vec3.dot(this.normal, w);
      ...
      if (point)
        GLMatrix.vec3.add(point, line.a, GLMatrix.vec3.scale(point, u, sI));
    ...
    intersectRay: function(origin, direction) {
      var numer = GLMatrix.vec3.dot(this.normal, origin) + this.d;
      var denom = GLMatrix.vec3.dot(this.normal, direction);
    ...
    intersectPlane: function(p, line) {
      ...
      if (line) {
        ...
        var iP = bufs.iP = bufs.iP || GLMatrix.vec3.create(); // intersection point
        switch(maxc) {
        ...
        GLMatrix.vec3.copy(line[0], iP);
        GLMatrix.vec3.add(line[1], iP, u);
      }
    ...
    initialize: function(points) {
       ...
       * direction [0,1,0].
       **/
      this.normal = GLMatrix.vec3.clone([0,1,0]);
    ...
    classifyVec3: function(O) {
      if (O.array) return GLMatrix.vec3.dot(this.normal, O.array) + this.d;
      else return GLMatrix.vec3.dot(this.normal, O) + this.d;
    },
    ...
    set: function() {
      if (arguments.length == 2) {
        GLMatrix.vec3.copy(this.normal, arguments[1]);
        this.d = -GLMatrix.vec3.dot(arguments[1], arguments[0]);
      } else {
        var tmp1 = this.normal, tmp2 = GLMatrix.vec3.create();
        var points = arguments;
        
        if (arguments.length != 3) points = arguments[0];
        if (typeof(points[0]) == 'object' && points[0].array) {
          GLMatrix.vec3.subtract(tmp1, points[1].array, points[0].array);
          GLMatrix.vec3.subtract(tmp2, points[2].array, points[0].array);
          GLMatrix.vec3.normalize(this.normal, vec3.cross(this.normal, tmp1, tmp2));
          this.d = -GLMatrix.vec3.dot(this.normal, points[0].array);
        } else {
          GLMatrix.vec3.subtract(tmp1, points[1], points[0]);
          GLMatrix.vec3.subtract(tmp2, points[2], points[0]);
          GLMatrix.vec3.normalize(this.normal, GLMatrix.vec3.cross(this.normal, tmp1, tmp2));
          this.d = -GLMatrix.vec3.dot(this.normal, points[0]);
        }
      }
      
      GLMatrix.vec3.scale(this.point, this.normal, this.d);
      return this;
    },
