    ...
    if (data) {
      for (attribute in data) {
        switch(attribute) {
          ...
          case 'mesh':
            if (data[attribute] instanceof Jax.Mesh) self.mesh = data[attribute];
    ...
    return Jax.Class.create({
      ...
      render: function(context, options) {
        if (this.mesh)
        {
          ...
          options = Jax.Util.merge(options, {model_index: this.__unique_id});
          context.pushMatrix(function() {
            context.matrix_stack.multModelMatrix(self.camera.getTransformationMatrix());
