...
Jax.Util = {
   ...
   * @return {Array}
   */
  trim_duplicates: function(array) {
    var contains, item, results, _i, _len;
    results = [];
    contains = function(haystack, needle) {
      var key, match, straw, value, _i, _j, _len, _len1;
      for (_i = 0, _len = haystack.length; _i < _len; _i++) {
        straw = haystack[_i];
        match = true;
        for (key = _j = 0, _len1 = needle.length; _j < _len1; key = ++_j) {
          value = needle[key];
          if (straw[key] !== value) {
            match = false;
          }
