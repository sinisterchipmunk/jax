...
Jax.Scene.LightManager = (function() {
  return Jax.Class.create({
    ...
    illuminate: function(context, material) {
      // Use alpha blending for the first pass, and additive blending for subsequent passes.
      context.gl.blendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
      var i;
      for (i = 0; i < this._lights.length; i++) {
        if (i == 1) context.gl.blendFunc(GL_ONE, GL_ONE);
        this._current_light = i;
        this._lights[i].render(context, this.objects, material);
      }
      
      if (i > 0) context.gl.blendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
