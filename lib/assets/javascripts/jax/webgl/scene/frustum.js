...
Jax.Scene.Frustum = (function() {
  ...
  var extentsM = mat4.create(), extentsInf = vec4.create();
  function extents(self)
  {
    var m = extentsM, inf = extentsInf;
    /* TODO see how this can be combined with Camera#unproject */
    function extent(x, y, z)
    {
      var mm = self.mv, pm = self.p;
      mat4.multiply(pm, mm, m);
      mat4.inverse(m, m);

      // Transformation of normalized coordinates between -1 and 1
      inf[0] = x;    /* x*2-1 translates x from 0..1 to -1..1 */
      inf[1] = y;
      inf[2] = z;
      inf[3] = 1;
  ...
  var klass = Jax.Class.create({
     ...
     **/
    sphere: function(center, radius, y, z)
    {
      if (!this.mv || !this.p) return INSIDE;
      if (!this.isValid()) this.validate();
      if (y !== undefined) {
        radius = z;
        center = [center, radius, y];
      }
