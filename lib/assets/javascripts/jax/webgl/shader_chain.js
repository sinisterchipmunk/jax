  ...
  return Jax.Class.create(Jax.Shader.Program, {
    ...
    link: function($super, context, material) {
      ...
      if (!program.linked) {
        ...
        if (!Jax.Shader.maximums_defined) {
          Jax.Shader.maximums_defined = true;
          Jax.Shader.max_varyings          = context.gl.getParameter(GL_MAX_VARYING_VECTORS);
          Jax.Shader.max_vertex_uniforms   = context.gl.getParameter(GL_MAX_VERTEX_UNIFORM_VECTORS);
          Jax.Shader.max_fragment_uniforms = context.gl.getParameter(GL_MAX_FRAGMENT_UNIFORM_VECTORS);
          Jax.Shader.max_attributes        = context.gl.getParameter(GL_MAX_VERTEX_ATTRIBS);
          Jax.Shader.max_vertex_textures   = context.gl.getParameter(GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS);
