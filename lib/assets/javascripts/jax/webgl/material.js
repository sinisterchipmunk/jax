...
Jax.Material = (function() {
  ...
  return Jax.Class.create({
    ...
    setShaderVariables: function(context, mesh, options, manifest) {
      ...
      if (this.setVariables)  this.setVariables(context, mesh, options, manifest);
      if (this.setUniforms)   this.setUniforms(context, mesh, options, manifest);
      if (this.setAttributes) this.setAttributes(context, mesh, options, manifest);

      for (var i = 0; i < this.layers.length; i++) {
        manifest.variable_prefix = this.layers[i].shader_variable_prefix;
        if (this.layers[i].setVariables)  this.layers[i].setVariables(context, mesh, options, manifest);
        if (this.layers[i].setUniforms)   this.layers[i].setUniforms(context, mesh, options, manifest);
        if (this.layers[i].setAttributes) this.layers[i].setAttributes(context, mesh, options, manifest);
      }
    },
