...
Jax.Material = (function() {
  ...
  function instantiate_layer(options) {
    if (options instanceof Jax.Material)
      return options;
    else {
      if (options.type) {
  ...
  return Jax.Class.create({
    initialize: function(options) {
      options = options || {};
      options.ambient  = options.ambient  || [1,1,1,1];
      options.diffuse  = options.diffuse  || [1,1,1,1];
      options.specular = options.specular || [1,1,1,1];
      options.emissive = options.emissive || [0,0,0,1];
      options.shininess = options.shininess === undefined ? 10 : options.shininess;
      options.default_shader = options.default_shader || Jax.default_shader;
      ...
      if (!options.shader && !options.default_shader)
        throw new Error("No shader specified for Jax.Material "+this.name);
    ...
    setShaderVariables: function(context, mesh, model, manifest) {
      manifest.variable_prefix = this.shader_variable_prefix;

      var stack = context.matrix_stack;
      manifest.set({
        mMatrix: stack.getModelMatrix(),
        vnMatrix: mat3.transpose(mat4.toMat3(stack.getViewMatrix())),
        ivMatrix:  stack.getInverseViewMatrix(),
        vMatrix:   stack.getViewMatrix(),
      ...
      if (this.setVariables)  this.setVariables(context, mesh, model, manifest);
      if (this.setUniforms)   this.setUniforms(context, mesh, model, manifest);
      if (this.setAttributes) this.setAttributes(context, mesh, model, manifest);

      for (var i = 0; i < this.layers.length; i++) {
        manifest.variable_prefix = this.layers[i].shader_variable_prefix;
        if (this.layers[i].setVariables)  this.layers[i].setVariables(context, mesh, model, manifest);
        if (this.layers[i].setUniforms)   this.layers[i].setUniforms(context, mesh, model, manifest);
        if (this.layers[i].setAttributes) this.layers[i].setAttributes(context, mesh, model, manifest);
      }
    },
     ...
     **/
    render: function(context, mesh, model) {
      ...
      try {
        ...
        shader.render(context, mesh, this, model);
      } catch(error) {
        if (error instanceof RangeError) {
          ...
          if (this.layers.length > 0) {
            ...
            this.render(context, mesh, model);
          }
          else throw error;
        }
        else throw error;
    ...
    adaptShaderToHardwareLimits: function(shader, error) {
      function log(msg) {
        if (Jax.getGlobal().ENV && Jax.getGlobal().ENV['SILENCED']) return;
...
Jax.Material.find = function(name) {
  var result;
  if (name instanceof Jax.Material) return name;
  if (result = Jax.Material.instances[name])
    return result;
  throw new Error("Material "+JSON.stringify(name)+" could not be found.");
};
