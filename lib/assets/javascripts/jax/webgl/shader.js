...
Jax.Shader = (function() {
  function buildStackTrace(context, glShader, source) {
    source = source.split(/\n/);
    var log = context.gl.getShaderInfoLog(glShader).split(/\n/);
  ...
  function compile(context, shader, source) {
    ...
    if (!context.gl.getShaderParameter(shader, GL_COMPILE_STATUS))
...
//      source = self.getGlobalDefinitions(options, isVertex) + source;
  ...
  return Jax.Class.create({
    ...
    getVertexShader: function(context) {
      ...
      this.shaders.vertex[context.id] = this.shaders.vertex[context.id] || context.gl.createShader(GL_VERTEX_SHADER);
      return this.shaders.vertex[context.id];
    },
    
    getFragmentShader: function(context) {
      if (!this.options.fragment) return null;
      this.shaders.fragment[context.id] = this.shaders.fragment[context.id] || context.gl.createShader(GL_FRAGMENT_SHADER);
