  ...
  return Jax.Class.create({
    initialize: function(context) {
      ...
      this._queueSorter = function(a, b) {
        var camPos = world._sortPosition;
    ...
    addObject: function(object, options) {
      ...
      if (object.mesh && object.cull && object.mesh.cull !== false && (!options || options.addToOctree !== false || options.cull === false)) {
     ...
     * - object (Jax.Model): the model instance to remove
     *
     * If the model has not been added to this World, nothing happens.
     * Otherwise, the object is removed from this World.
     *
     * The object itself is returned.
     **/
    removeObject: function(obj) {
      delete this.objects[obj.__unique_id];
      this._objectsArray.splice(this._objectsArray.indexOf(obj), 1);
    ...
    */
    ...
    render: function(material, cull) {
      ...
      vec3.scale(this._sortPosition, 0);
      mat4.multiplyVec3(context.matrix_stack.getInverseModelViewMatrix(), this._sortPosition);

      if (cull !== false) {
        var frustum = this.context.player.camera.getFrustum();
        var renderOctree = this.renderOctree, _octreeModel = this._octreeModel;
        this.octree.traverse(this._sortPosition, function(node) {
          ...
          if (renderOctree && objectCount)
            node.mesh.render(context, _octreeModel, material);
          for (i in objects) {
            o = objects[i];
            // transparencies need to be queued so they can be rendered last
            if (o.transparent || (o.mesh && o.mesh.transparent)) queue.push(o);
            else o.render(context, material);
          }

          return keepRecursing;
        });
      } else {
        for (i in this.octree.nestedObjects) {
          var obj = this.octree.nestedObjects[i];
          if (obj.transparent || (obj.mesh && obj.mesh.transparent)) queue.push(obj);
          else obj.render(context, material);
        }
      }
